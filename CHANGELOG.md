...
### ⚠ Breaking changes:
  * The `duration` parameter has been moved from `rollTo` into `CurveAnimationConfig`. When no `AnimationConfig` is provided, the default `CurveAnimationConfig` (with default duration and curve) is used automatically.
